...
func main() {
	...
	requestTimeout := flag.Duration("requestTimeout", 1*time.Second, "Timeout for single request")
	...
	sg, err := lsdalm.NewStreamLoader(*name, *url, *pollTime, logger, *sessions, *restarts, *singleconn, *requestTimeout)
	if err != nil {
